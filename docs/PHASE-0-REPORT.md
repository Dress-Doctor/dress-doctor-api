...
| 4.9 Containerization | None | `Dockerfile` (multi-stage, one image/two entrypoints) + `docker-compose.yml` (mongo, redis, api, worker) added — **unverified**, see §4 below |
...
- **Docker is unverified.** No `docker` binary was available in this sandbox. `Dockerfile` and `docker-compose.yml` are written and reviewed against the app's actual runtime requirements (including the `.hbs` template path quirk — `NotificationService` resolves templates from `${cwd}/src/static`, not `dist`, so the runtime image stage ships `src/static` alongside `dist`), but `docker compose up` itself has not been run. **Needs a real docker-enabled environment before relying on this as proven.**
...
- [ ] `docker compose up` — written, **not run** (no docker in this sandbox)
