import { ApiProperty } from '@nestjs/swagger';
import {
  ApiSuccessResponse,
  ApiSuccessResponseWithPagination,
} from 'src/dto/swagger.dto';
import { CatalogueHistoryEntryEntity } from './catalogue.entity';

export class ServiceTypeEntity {
  @ApiProperty({ example: '64b8c9f1e4b0a2d3c4f5g6h' })
  _id: string;

  @ApiProperty({
    example: 'ST-A4F92C',
    description:
      'Human-readable identifier. Every read and write of a single service type ' +
      'is addressed by this, never by the mongo id.',
  })
  reference: string;

  @ApiProperty({ example: 'Heavy' })
  serviceTypeName: string;

  @ApiProperty({ required: false, example: 'Heavy duty cleaning service' })
  description?: string;

  @ApiProperty({ default: true, example: true })
  isActive: boolean;

  @ApiProperty({ example: '2023-07-21T12:00:00Z' })
  createdAt: string;

  @ApiProperty({ example: '2023-07-21T12:00:00Z' })
  updatedAt: string;
}

export class FindAllServiceTypeEntity extends ApiSuccessResponseWithPagination {
  @ApiProperty({ type: [ServiceTypeEntity] })
  data: ServiceTypeEntity[];
}

export class ServiceTypeDetailDataEntity extends ServiceTypeEntity {
  @ApiProperty({
    example: 8,
    description: 'How many items are priced under this service type.',
  })
  itemCount: number;

  @ApiProperty({
    type: [CatalogueHistoryEntryEntity],
    description:
      'Audit trail, newest first. The stored snapshot is never returned.',
  })
  history: CatalogueHistoryEntryEntity[];
}

export class ServiceTypeDetailEntity extends ApiSuccessResponse {
  @ApiProperty({ type: ServiceTypeDetailDataEntity })
  data: ServiceTypeDetailDataEntity;
}
