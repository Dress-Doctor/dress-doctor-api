...
describe('OrderController', () => {
  ...
  beforeEach(async () => {
    service = {
      ...
      findAllWithItems: jest.fn().mockResolvedValue({ total: 0, data: [], nextPage: null }),
    };
  ...
  it('getOrders should call service with query', async () => {
    ...
    expect(service.findAllWithItems).toHaveBeenCalledWith({ page: 1, size: 10 });
  });

  it('createOrder should call service', async () => {
    const req: any = { data: { platform: 'p' }, user: { phone: '123' } };
    const body: any = { customerId: 'a', currencyId: 'b', pickupRequestId: 'c' };
