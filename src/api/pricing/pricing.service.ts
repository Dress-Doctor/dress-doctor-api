...
export class PricingService {
   ...
   * Throws PRICE_NOT_FOUND when neither exists.
   */
  ...
  /**
   * Compute a quote (per-line + subtotal + applicable promo) WITHOUT creating
   * an order. This is the single pricing engine both order intake and booking
   * rely on. Per-customer promo limits are enforced at order creation (§3.5),
   * where the customer is known.
   */
  async quote(data: QuoteDto): Promise<QuoteResult> {
    this.can('READ', 'Item');
    ...
    for (const line of data.items) {
      const itemId = new Types.ObjectId(line.itemId);
      const serviceTypeId = new Types.ObjectId(line.serviceTypeId);
      const price = await this.resolvePrice(itemId, serviceTypeId, officeId);
    ...
    if (data.promoCode) {
      discount = await this.applyPromo(
        data.promoCode,
        subtotal,
        data.items.map((i) => i.serviceTypeId),
      );
      promoApplied = { code: data.promoCode, discount };
    }
  ...
  /**
   * Validate a promo against the quote and return the XAF discount. Pure money
   * math — integer XAF, percentage floored. Does not enforce per-customer caps.
   */
  private async applyPromo(
    ...
    if (promo.applicableServiceTypeIds.length > 0) {
      ...
      const applies = lineServiceTypeIds.every((id) => allowed.has(id));
      if (!applies) {
        throw new BadRequestException({
  ...
  async createPrice(data: CreatePriceDto) {
    ...
    // Append-only: deactivate the prior active price for this scope, so
    // resolvePrice always reads the current row.
