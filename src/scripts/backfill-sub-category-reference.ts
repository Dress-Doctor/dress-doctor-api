/**
 * One-shot backfill: give every pre-existing sub category a `reference`.
 *
 ...
 *
 * Run:  npm run backfill:sub-category-reference
 */
import { Logger } from '@nestjs/common';
import { getModelToken } from '@nestjs/mongoose';
import { Model } from 'mongoose';
import { CodeGeneratorService } from './../helper/service/code-generator.service';
import { SubCategory } from './../schema/catalog/sub-category.schema';
import { createScriptContext } from './script-context';

async function backfill() {
  const logger = new Logger('BackfillSubCategoryReference');
  ...
  try {
    ...
    const total = await subCategoryModel.countDocuments(missing);
    logger.log(`${total} sub category(s) without a reference`);

    let done = 0;
    const cursor = subCategoryModel.find(missing).select('_id').lean().cursor();
    for await (const row of cursor) {
      const reference = await codeService.generateSubCategoryReference();
    ...
    logger.log(`✅ Backfilled ${done} sub category reference(s)`);
  } finally {
    await app.close();
  }
}
...
backfill().catch((error) => {
  new Logger('BackfillSubCategoryReference').error(error);
  process.exit(1);
});
