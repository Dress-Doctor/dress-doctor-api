/**
 * One-shot backfill: give every pre-existing item a `reference`.
 *
 ...
 *
 * Run:  npm run backfill:item-reference
 */
...
async function backfill() {
  const logger = new Logger('BackfillItemReference');
  ...
  try {
    ...
    const total = await itemModel.countDocuments(missing);
    logger.log(`${total} item(s) without a reference`);

    let done = 0;
    const cursor = itemModel.find(missing).select('_id').lean().cursor();
    for await (const row of cursor) {
      const reference = await codeService.generateItemReference();
    ...
    logger.log(`✅ Backfilled ${done} item reference(s)`);
  } finally {
    await app.close();
  }
}
...
backfill().catch((error) => {
  new Logger('BackfillItemReference').error(error);
  process.exit(1);
});
