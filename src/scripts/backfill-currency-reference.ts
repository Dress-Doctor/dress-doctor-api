/**
 * One-shot backfill: give every pre-existing currency a `reference`.
 *
 ...
 *
 * Run:  npm run backfill:currency-reference
 */
...
async function backfill() {
  const logger = new Logger('BackfillCurrencyReference');
  const app = await createScriptContext();

  try {
    const currencyModel = app.get<Model<Currency>>(
      getModelToken(Currency.name),
    );
    ...
    const total = await currencyModel.countDocuments(missing);
    logger.log(`${total} currency(s) without a reference`);

    let done = 0;
    const cursor = currencyModel.find(missing).select('_id').lean().cursor();
    for await (const row of cursor) {
      const reference = await codeService.generateCurrencyReference();
    ...
    logger.log(`✅ Backfilled ${done} currency reference(s)`);
  } finally {
    await app.close();
  }
}
...
backfill().catch((error) => {
  new Logger('BackfillCurrencyReference').error(error);
  process.exit(1);
});
