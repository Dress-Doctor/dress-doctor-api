...
export const categoryHistorySchemaName = 'category_history';

/**
 * The audit trail of one category row — its name, description and status.
 *
 ...
 */
@Schema({ timestamps: true, collection: categoryHistorySchemaName })
export class CategoryHistory extends Document<Types.ObjectId> {
  @Prop({
    index: true,
    required: true,
    type: Types.ObjectId,
...
export const CategoryHistorySchema =
  SchemaFactory.createForClass(CategoryHistory);
