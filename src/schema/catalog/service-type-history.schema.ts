...
export const serviceTypeHistorySchemaName = 'service_type_history';

/**
 * The audit trail of one service-type row — its name, description and status.
 *
 ...
 */
@Schema({ timestamps: true, collection: serviceTypeHistorySchemaName })
export class ServiceTypeHistory extends Document<Types.ObjectId> {
  @Prop({
    index: true,
    required: true,
    type: Types.ObjectId,
    ref: ServiceType.name,
  })
...
export const ServiceTypeHistorySchema =
  SchemaFactory.createForClass(ServiceTypeHistory);
